...
import { useSocket } from '../../context/SocketProvider/Socket';
import { AuthContext } from '../../context/AuthProvider/AuthProvider';

const AllLiveClass = ({ allLiveClass }) => {
    const {socket} = useSocket();
    const {user} = useContext(AuthContext);
    console.log(user)
    
    const { _id, name, bgImage, ratings, star, people, position, person } = allLiveClass;
    const handleJoinClass = () =>{
        socket.emit('join-class',{email:user.email,_id})
            }
    return (
