...
const JobPreparation = () => {
    ...
    const handleCVChange = async (event) => {
      ...
      formData.append('cv', file); // 'cv' is the key used to access the file on the server side
  
      try {
        const response = await fetch('http://your-backend-url/upload-cv', {
          method: 'POST',
          body: formData,
        });
        ...
        if (response.ok) {
          ...
          // File upload failed
          console.error('CV upload failed:', response.statusText);
        }
      } catch (error) {
    ...
    return (
        ...
        style={{ display: 'none' }}
      />
                    <button onClick={handleUploadCV} className="flex gap-2 font-bold border-0 text-white text-[12px] px-6 py-2 rounded-lg items-center mt-8" style={{ background: 'linear-gradient(135deg, #92FFC0 0%, #002661 100%)' }}>
                        <p>Upload CV</p>
